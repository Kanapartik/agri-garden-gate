...
async function tenantScope(supabase: any, userId: string, tenantId: string) {
  ...
  const permitted =
    actor.isPlatformAdmin || actor.isAuditor || actor.tenantIds.includes(tenantId);
...
export const getMemberRegistry = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<MemberRegistry> => {
    ...
    const [{ data: members }, { data: tags }, { data: assignments }, { data: segments }, { data: consents }] =
      await Promise.all([
        supabase
          .from("fpo_members")
          .select(
    ...
    const tagRows = (tags ?? []) as Array<{ id: string; code: string; label: string; description: string | null; color: string | null }>;
...
export const saveMember = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.memberId) {
      ...
      if (existing.tenant_id !== data.tenantId) throw new Error("Member belongs to another organization");
    ...
    const { data: existingNumbers } = await supabase
      ...
      .eq("tenant_id", data.tenantId);
    const rows = (existingNumbers ?? []) as Array<{ membership_number: string | null; member_ref: string }>;
    ...
    const number = membershipNumber((profile as { fpo_code?: string } | null)?.fpo_code ?? "FPO", seq);
    const ref = data.member_ref?.trim() || `M-${String(seq).padStart(4, "0")}`;
    if (rows.some((r) => r.member_ref === ref)) throw new Error("That member reference already exists");
...
export const setMembershipStatus = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if ((data.status === "exited" || data.status === "removed") && !canClassifyMembers(roles, actor.isPlatformAdmin)) {
...
export const searchFarmerCandidates = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<FarmerCandidate[]> => {
    ...
    const { data: rows } = await supabaseAdmin
      ...
      .or(`full_name.ilike.%${term.replace(/[%,]/g, "")}%,village_code.ilike.%${term.replace(/[%,]/g, "")}%`)
      .limit(10);
...
export const saveTag = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator(
    (input: { tenantId: string; label: string; description?: string | null; color?: string | null }) =>
      input,
  )
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
...
export const assignTag = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator(
    (input: { tenantId: string; tagId: string; memberIds: string[]; mode: "add" | "remove" }) => input,
  )
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    const { actor, roles } = await tenantScope(supabase, userId, data.tenantId);
...
export const saveSegment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const query = data.segmentId
      ? supabase.from("fpo_member_segments").update(payload).eq("id", data.segmentId).select("id").single()
...
export const revokeMemberConsent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const row = consent as { id: string; tenant_id: string; farmer_user_id: string; purpose_code: string };
...
export const getFarmer360 = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<Farmer360> => {
    ...
    const purposes = activePurposes((consentRows ?? []) as Array<{ purpose_code: string; revoked_at: string | null; expires_at: string | null }>);
